...
public class RectangleMenu extends MTRectangle implements CSSStylableComponent{
	...
	private void styleChildren(int fontsize) {
		...
		for (MTRectangle c : menuContents) {
			...
			c.setHeightLocal(size/2); //changed size to size/2
			...
			if (((MTRectangle) c).getTexture() == null) {
				...
				float ypos = size / 4f - height / 2f; //changed size / 2f to size / 4f
				for (MTComponent d : c.getChildren()) {
					if (d instanceof MTTextArea) {
						MTTextArea ta = (MTTextArea) d;
	...
	public List<MTRectangle> getMenuContent() {
		return menuContents;
	}
			...
			if (ge instanceof TapEvent) {
				...
				if (te.getTapID() == TapEvent.TAPPED) {
					...
					for (PolygonListeners pl: children) {
						...
						if (pl.component.getIntersectionGlobal(te.getCursor()) != null) {
							close();
